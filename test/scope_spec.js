...
describe('Scope', function(){
	...
	describe('$applyAsync', function(){
		...
		it('allows asyc $apply with $applyAsync', function(done){
			scope.counter = 0;
			scope.$watch(function(scope){
				return scope.aValue;
			},function(newValue, oldValue, scope){
	...
	describe('digest',function(){
		...
		it('calls listener when watch value is first undefined', function(){
			scope.counter = 0;
			scope.$watch(function(){ return scope.someValue; },
			function(newValue, oldValue, scope){ scope.counter++;});
		...
		it('allows $stateful filter value to change over time', function () {
			scope = createInjector(['ng', function ($filterProvider) {
	...
	describe('$watchCollection', function () {
		...
		it('does not fail on NaN attributes in obejects', function () {
			scope.counter = 0;
			scope.obj = {a: NaN};
		...
		it('givens the old object value to listeners', function () {
	...
	describe('Events', function () {
		...
		_.forEach(['$emit', '$broadcast'], function (method) {
			it('calls listeners registered for matching event on' + method, function () {
        ...
        it('is received by listeners on current scope after being stopped', function () {
			var listened1 = function (event) {
				event.stopPropagation();
            };
			var listener2 = jasmine.createSpy();
			scope.$on('someEvent',listened1);
			scope.$on('someEvent',listener2);
			scope.$emit('someEvent');
			expect(listener2).toHaveBeenCalled();
        });
